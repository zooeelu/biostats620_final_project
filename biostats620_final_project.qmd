...
```
Comparing this model with the log-linear model, we can see that the residual deviance has been reduced from 66290 to 16776 on 15858 degrees of freedom, showing that this model has a better fit than the log-linear model. However, the probability of observing such a residual deviance value given that the fit model holds is still very low, indicating a lack of model fit desipite its improvements over the previous model.
