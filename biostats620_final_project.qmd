...
# 2017 and after data used for prediction
...
ggplot(data.frame(residuals = resids), aes(sample = residuals)) +
  stat_qq(size = 1.5, color = "darkblue", alpha = 0.6) +      # dots
  stat_qq_line(color = "orange", size = 1) +  # reference line
...
In addition to the model violating the linear model assumptions, we can see that the model does not predcit well and a better suited model should be used. From the Predicted vs Observed Rates plot, we can see that for the most part, the linear model is overestimating the actual mortality rate. There seems to be more week to week variation in actual mortality for the 65-84 and 85+ age groups which the model is also not fully capturing.
...
## Comparison of NYT Data and excessmort Data
```{r}
...
# Combine all pages into one string and split into lines
all_text <- unlist(strsplit(paste(pdf_text, collapse = "\n"), "\n"))

# Filter lines that look like dates (start with YYYY-MM-DD)
date_lines <- grep("^\\d{4}-\\d{2}-\\d{2}", all_text, value = TRUE)

# Use regular expressions to extract date and count
mort_data <- str_match(date_lines, "^(\\d{4}-\\d{2}-\\d{2})\\s+(\\d+)")

# Convert to data frame
nyt_df <- data.frame(
  date = as.Date(mort_data[, 2]),
  deaths = as.numeric(mort_data[, 3])
)
